...
class SpacecraftThrusterModelPlugin(Plugin):
    def __init__(
        ...
        super().__init__(sdf_version=None, filename="gz-sim-spacecraft-thruster-model-system", name="gz::sim::systems::SpacecraftThrusterModel")
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        return cls(
            ...
            max_thrust=float(max_thrust_el.text) if max_thrust_el is not None and max_thrust_el.text is not None else None,
            duty_cycle_frequency=float(duty_cycle_frequency_el.text) if duty_cycle_frequency_el is not None and duty_cycle_frequency_el.text is not None else None,
            sub_topic=sub_topic_el.text if sub_topic_el is not None and sub_topic_el.text is not None else None,
        )

    def to_sdf(self, version: str | None = None) -> ET.Element:
        el = ET.Element("plugin", name=self.name if hasattr(self, 'name') else "gz::sim::systems::SpacecraftThrusterModel", filename="gz-sim-spacecraft-thruster-model-system")
