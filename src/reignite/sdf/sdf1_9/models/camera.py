...
class Camera(_PrevCamera):
    def __init__(
        ...
        super().__init__(name=name, pose=pose, camera_info_topic=camera_info_topic, horizontal_fov=horizontal_fov, image=image, clip=clip, save=save, depth_camera=depth_camera, noise=noise, distortion=distortion, lens=lens, visibility_mask=visibility_mask, optical_frame_id=optical_frame_id)
    ...
    def from_sdf(cls, el: ET.Element) -> "Camera":
        ...
        _segmentation_type = SegmentationType.from_sdf(_c_segmentation_type) if _c_segmentation_type is not None else None
        _c_box_type = el.find("box_type")
        _box_type = BoxType.from_sdf(_c_box_type) if _c_box_type is not None else None
        return cls(name=_base.name, pose=_base.pose, camera_info_topic=_base.camera_info_topic, triggered=_triggered, trigger_topic=_trigger_topic, horizontal_fov=_base.horizontal_fov, image=_base.image, clip=_base.clip, save=_base.save, depth_camera=_base.depth_camera, segmentation_type=_segmentation_type, box_type=_box_type, noise=_base.noise, distortion=_base.distortion, lens=_base.lens, visibility_mask=_base.visibility_mask, optical_frame_id=_base.optical_frame_id)
