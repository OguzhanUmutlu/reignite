...
class AudioSource(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        if _c_uri is not None:
            ...
            _uri = None
        return cls(sdf_version=version, contact=_contact, gain=_gain, loop=_loop, pitch=_pitch, pose=_pose, uri=_uri)
...
class Camera(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        return cls(sdf_version=version, box_type=_box_type, camera_info_topic=_camera_info_topic, clip=_clip, depth_camera=_depth_camera, distortion=_distortion, horizontal_fov=_horizontal_fov, image=_image, lens=_lens, name=_name, noise=_noise, optical_frame_id=_optical_frame_id, pose=_pose, save=_save, segmentation_type=_segmentation_type, trigger_topic=_trigger_topic, triggered=_triggered, visibility_mask=_visibility_mask)
...
class Collision(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        if _c_surface is not None:
            ...
            _surface = None
        return cls(sdf_version=version, auto_inertia_params=_auto_inertia_params, density=_density, geometry=_geometry, laser_retro=_laser_retro, mass=_mass, max_contacts=_max_contacts, name=_name, origin=_origin, pose=_pose, surface=_surface)
...
class DepthCamera(BaseModel):
    def __init__(self, sdf_version: str, clip: "DepthCameraClip" = None, output: str = "depths"):
        self.__version__ = sdf_version
        self.clip = clip
        self.output = output
...
class DepthCameraClip(BaseModel):
    def __init__(self, sdf_version: str, far: "ClipFar" = None, near: "Near" = None):
        self.__version__ = sdf_version
        self.far = far
        self.near = near
...
class Inertial(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        if _pose is not None and cmp_version(version, "1.2") < 0:
            return SDFError(f"'pose' is not supported in SDF version {version} (added in 1.2)")
        return cls(sdf_version=version, auto=_auto, auto_inertia_params=_auto_inertia_params, density=_density, fluid_added_mass=_fluid_added_mass, inertia=_inertia, mass=_mass, origin=_origin, pose=_pose)
...
class Light(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        if _c_visualize is not None:
            ...
            _visualize = None
        return cls(sdf_version=version, attenuation=_attenuation, cast_shadows=_cast_shadows, diffuse=_diffuse, direction=_direction, intensity=_intensity, light_on=_light_on, name=_name, pose=_pose, specular=_specular, spot=_spot, type=_type, visualize=_visualize)
...
class Link(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        return cls(sdf_version=version, acceleration=_acceleration, audio_sink=_audio_sink, audio_source=_audio_source, battery=_battery, collision=_collision, damping=_damping, enable_wind=_enable_wind, frame=_frame, gravity=_gravity, inertial=_inertial, kinematic=_kinematic, light=_light, must_be_base_link=_must_be_base_link, name=_name, origin=_origin, particle_emitter=_particle_emitter, pose=_pose, projector=_projector, self_collide=_self_collide, sensor=_sensor, velocity=_velocity, velocity_decay=_velocity_decay, visual=_visual, wrench=_wrench)


class LinkDamping(BaseModel):
    def __init__(self, sdf_version: str, angular: "Angular" = None, linear: "Linear" = None):
        self.__version__ = sdf_version
        self.angular = angular
        self.linear = linear
...
class Pose(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        if _rotation_format is not None and cmp_version(version, "1.12") < 0:
            if _rotation_format != "euler_rpy":
                return SDFError(f"'rotation_format' is not supported in SDF version {version} (added in 1.12)")
        return cls(sdf_version=version, degrees=_degrees, pose=_pose, rotation_format=_rotation_format)
...
class Projector(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        return cls(sdf_version=version, far_clip=_far_clip, fov=_fov, name=_name, near_clip=_near_clip, plugin=_plugin, pose=_pose, texture=_texture, visibility_flags=_visibility_flags)
...
class Sensor(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        return cls(sdf_version=version, air_pressure=_air_pressure, air_speed=_air_speed, altimeter=_altimeter, always_on=_always_on, camera=_camera, contact=_contact, enable_metrics=_enable_metrics, force_torque=_force_torque, frame_id=_frame_id, gps=_gps, imu=_imu, lidar=_lidar, logical_camera=_logical_camera, magnetometer=_magnetometer, name=_name, navsat=_navsat, origin=_origin, plugin=_plugin, pose=_pose, ray=_ray, rfid=_rfid, rfidtag=_rfidtag, sonar=_sonar, topic=_topic, transceiver=_transceiver, type=_type, update_rate=_update_rate, visualize=_visualize)
...
class Visual(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        if _visibility_flags is not None and cmp_version(version, "1.12") < 0:
            return SDFError(f"'visibility_flags' is not supported in SDF version {version} (added in 1.12)")
        return cls(sdf_version=version, cast_shadows=_cast_shadows, geometry=_geometry, laser_retro=_laser_retro, material=_material, meta=_meta, name=_name, origin=_origin, plugin=_plugin, pose=_pose, transparency=_transparency, visibility_flags=_visibility_flags)
