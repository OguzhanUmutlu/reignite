...
class Geometry(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        return cls(sdf_version=version, box=_box, sphere=_sphere, cylinder=_cylinder, mesh=_mesh, plane=_plane, image=_image, heightmap=_heightmap, empty=_empty, ellipsoid=_ellipsoid, cone=_cone, capsule=_capsule, polyline=_polyline)
...
class Contact(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        if _elastic_modulus is not None and cmp_version(version, "1.12") < 0:
            return SDFError(f"'elastic_modulus' is not supported in SDF version {version} (added in 1.12)")
        return cls(sdf_version=version, ode=_ode, collide_bitmask=_collide_bitmask, bullet=_bullet, collide_without_contact_bitmask=_collide_without_contact_bitmask, collide_without_contact=_collide_without_contact, category_bitmask=_category_bitmask, poissons_ratio=_poissons_ratio, elastic_modulus=_elastic_modulus)
...
class Camera(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        if _visibility_mask is not None and cmp_version(version, "1.12") < 0:
            return SDFError(f"'visibility_mask' is not supported in SDF version {version} (added in 1.12)")
        return cls(sdf_version=version, name=_name, horizontal_fov=_horizontal_fov, image=_image, clip=_clip, save=_save, depth_camera=_depth_camera, pose=_pose, noise=_noise, lens=_lens, segmentation_type=_segmentation_type, optical_frame_id=_optical_frame_id, box_type=_box_type, camera_info_topic=_camera_info_topic, triggered=_triggered, trigger_topic=_trigger_topic, distortion=_distortion, visibility_mask=_visibility_mask)
...
class Imu(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        return cls(sdf_version=version, topic=_topic, noise=_noise, enable_orientation=_enable_orientation, linear_acceleration=_linear_acceleration, orientation_reference_frame=_orientation_reference_frame, angular_velocity=_angular_velocity)
...
class Link(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        return cls(sdf_version=version, name=_name, gravity=_gravity, self_collide=_self_collide, kinematic=_kinematic, inertial=_inertial, collision=_collision, visual=_visual, sensor=_sensor, projector=_projector, origin=_origin, damping=_damping, pose=_pose, velocity_decay=_velocity_decay, audio_source=_audio_source, must_be_base_link=_must_be_base_link, audio_sink=_audio_sink, frame=_frame, wrench=_wrench, acceleration=_acceleration, velocity=_velocity, enable_wind=_enable_wind, battery=_battery, particle_emitter=_particle_emitter, light=_light)
