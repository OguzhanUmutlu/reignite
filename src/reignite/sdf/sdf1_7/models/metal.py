...
from ...sdf1_6.models.metal import Metal as _PrevMetal
...
class Metal(_PrevMetal):
    def __init__(
        ...
        super().__init__(albedo_map=albedo_map, roughness_map=roughness_map, roughness=roughness, metalness_map=metalness_map, metalness=metalness, environment_map=environment_map, ambient_occlusion_map=ambient_occlusion_map, normal_map=normal_map, emissive_map=emissive_map)
        self.light_map = light_map

    def to_sdf(self) -> ET.Element:
        el = super().to_sdf()
        if self.light_map is not None:
            el.append(self.light_map.to_sdf())
        return el
    ...
    def from_sdf(cls, el: ET.Element) -> "Metal":
        ...
        _light_map = LightMap.from_sdf(_c_light_map) if _c_light_map is not None else None
