...
from ...sdf1_9.models.sensor import Sensor as _PrevSensor
...
class Sensor(_PrevSensor):
    def __init__(
        ...
        super().__init__(name=name, type=type, pose=pose, plugin=plugin, air_pressure=air_pressure, altimeter=altimeter,
                         camera=camera, contact=contact, force_torque=force_torque, gps=gps, imu=imu, lidar=lidar,
                         logical_camera=logical_camera, magnetometer=magnetometer, navsat=navsat, ray=ray,
                         rfidtag=rfidtag, rfid=rfid, sonar=sonar, transceiver=transceiver, always_on=always_on,
    ...
    def to_sdf(self) -> ET.Element:
        ...
        if self.air_speed is not None:
            el.append(self.air_speed.to_sdf())
        return el
    ...
    def from_sdf(cls, el: ET.Element) -> "Sensor":
        ...
        _air_speed = AirSpeed.from_sdf(_c_air_speed) if _c_air_speed is not None else None
        return cls(name=_base.name, type=_base.type, pose=_base.pose, plugin=_base.plugin,
                   air_pressure=_base.air_pressure, air_speed=_air_speed, altimeter=_base.altimeter,
