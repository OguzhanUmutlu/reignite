...
class Geometry(_PrevGeometry):
    def __init__(
        ...
        super().__init__(box=box, cylinder=cylinder, heightmap=heightmap, image=image, mesh=mesh, plane=plane, sphere=sphere, empty=empty)
    ...
    def from_sdf(cls, el: ET.Element) -> "Geometry":
        ...
        return cls(box=_base.box, cylinder=_base.cylinder, heightmap=_base.heightmap, image=_base.image, mesh=_base.mesh, plane=_base.plane, polyline=_polyline, sphere=_base.sphere, empty=_base.empty)
