from __future__ import annotations

from typing import List
from xml.etree import ElementTree as ET

from .animation import Animation
from .joint import Joint
from .link import Link
from .plugin import Plugin
from .script import Script
from .skin import Skin
from ...sdf1_5.models.actor import Actor as _PrevActor
from ...sdf1_5.models.frame import Frame as _PrevFrame
from ...sdf1_5.models.pose import Pose as _PrevPose
from ...sdf1_5.models.static import Static as _PrevStatic
from ....utils.pose import Pose


class Pose(_PrevPose):
    def __init__(self, pose: Pose = None, frame: str = ""):
        if pose is None:
            pose = Pose.from_sdf("0 0 0 0 0 0")
        super().__init__(pose=pose, frame=frame)

    def to_sdf(self) -> ET.Element:
        el = super().to_sdf()
        return el

    @classmethod
    def from_sdf(cls, el: ET.Element) -> "Pose":
        _base = _PrevPose.from_sdf(el)
        return cls(pose=_base.pose, frame=_base.frame)


class Frame(_PrevFrame):
    def __init__(self, name: str = "", pose: "Pose" = None):
        super().__init__(name=name, pose=pose)

    def to_sdf(self) -> ET.Element:
        el = super().to_sdf()
        return el

    @classmethod
    def from_sdf(cls, el: ET.Element) -> "Frame":
        _base = _PrevFrame.from_sdf(el)
        return cls(name=_base.name, pose=_base.pose)


class Static(_PrevStatic):
    def __init__(self, static: bool = False):
        super().__init__(static=static)

    def to_sdf(self) -> ET.Element:
        el = super().to_sdf()
        return el

    @classmethod
    def from_sdf(cls, el: ET.Element) -> "Static":
        _base = _PrevStatic.from_sdf(el)
        return cls(static=_base.static)


class Actor(_PrevActor):
    def __init__(
            self,
            name: str = "__default__",
            frame: List["Frame"] = None,
            pose: "Pose" = None,
            link: List["Link"] = None,
            joint: List["Joint"] = None,
            plugin: List["Plugin"] = None,
            static: "Static" = None,
            skin: "Skin" = None,
            animation: List["Animation"] = None,
            script: "Script" = None
    ):
        super().__init__(name=name, frame=frame, pose=pose, link=link, joint=joint, plugin=plugin, static=static,
                         skin=skin, animation=animation, script=script)

    def to_sdf(self) -> ET.Element:
        el = super().to_sdf()
        return el

    @classmethod
    def from_sdf(cls, el: ET.Element) -> "Actor":
        _base = _PrevActor.from_sdf(el)
        return cls(name=_base.name, frame=_base.frame, pose=_base.pose, link=_base.link, joint=_base.joint,
                   plugin=_base.plugin, static=_base.static, skin=_base.skin, animation=_base.animation,
                   script=_base.script)
