...
class Material(_PrevMaterial):
    def __init__(
        ...
        super().__init__(script=script, shader=shader, lighting=lighting, ambient=ambient, diffuse=diffuse, specular=specular, emissive=emissive)
        self.pbr = pbr
    ...
    def from_sdf(cls, el: ET.Element) -> "Material":
        ...
        return cls(script=_base.script, shader=_base.shader, lighting=_base.lighting, ambient=_base.ambient, diffuse=_base.diffuse, specular=_base.specular, emissive=_base.emissive, pbr=_pbr)
