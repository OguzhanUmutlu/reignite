...
from ...sdf1_5.models.sensor import Sensor as _PrevSensor
...
class Sensor(_PrevSensor):
    def __init__(
        ...
        super().__init__(name=name, type=type, frame=frame, pose=pose, plugin=plugin, altimeter=altimeter,
                         camera=camera, contact=contact, force_torque=force_torque, gps=gps, imu=imu,
    ...
    def to_sdf(self) -> ET.Element:
        ...
        if self.air_pressure is not None:
            el.append(self.air_pressure.to_sdf())
        if self.lidar is not None:
            el.append(self.lidar.to_sdf())
        return el
    ...
    def from_sdf(cls, el: ET.Element) -> "Sensor":
        ...
        _air_pressure = AirPressure.from_sdf(_c_air_pressure) if _c_air_pressure is not None else None
        _c_lidar = el.find("lidar")
        _lidar = Lidar.from_sdf(_c_lidar) if _c_lidar is not None else None
        return cls(name=_base.name, type=_base.type, frame=_base.frame, pose=_base.pose, plugin=_base.plugin,
                   air_pressure=_air_pressure, altimeter=_base.altimeter, camera=_base.camera, contact=_base.contact,
                   force_torque=_base.force_torque, gps=_base.gps, imu=_base.imu, lidar=_lidar,
                   logical_camera=_base.logical_camera, magnetometer=_base.magnetometer, ray=_base.ray,
                   rfidtag=_base.rfidtag, rfid=_base.rfid, sonar=_base.sonar, transceiver=_base.transceiver,
                   always_on=_base.always_on, update_rate=_base.update_rate, visualize=_base.visualize,
                   topic=_base.topic)
