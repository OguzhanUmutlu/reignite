...
from ...sdf1_5.models.heightmap import Heightmap as _PrevHeightmap
from .uri import Uri
...
class Heightmap(_PrevHeightmap):
    ...
    def to_sdf(self) -> ET.Element:
        ...
        if self.sampling is not None:
            el.append(self.sampling.to_sdf())
        return el
    ...
    def from_sdf(cls, el: ET.Element) -> "Heightmap":
        ...
        _sampling = Sampling.from_sdf(_c_sampling) if _c_sampling is not None else None
        return cls(uri=_base.uri, size=_base.size, pos=_base.pos, texture=_base.texture, blend=_base.blend, use_terrain_paging=_base.use_terrain_paging, sampling=_sampling)
