...
class World(_PrevWorld):
    def __init__(
        ...
        super().__init__(name=name, gui=gui, physics=physics, scene=scene, light=light, model=model, actor=actor, plugin=plugin, joint=joint, road=road, spherical_coordinates=spherical_coordinates, state=state, population=population, audio=audio, include=include)
    ...
    def from_sdf(cls, el: ET.Element) -> "World":
        ...
        return cls(name=_base.name, atmosphere=_atmosphere, gui=_base.gui, physics=_base.physics, scene=_base.scene, light=_base.light, model=_base.model, actor=_base.actor, plugin=_base.plugin, joint=_base.joint, road=_base.road, spherical_coordinates=_base.spherical_coordinates, state=_base.state, population=_base.population, audio=_base.audio, wind=_wind, include=_base.include, gravity=_gravity, magnetic_field=_magnetic_field)
