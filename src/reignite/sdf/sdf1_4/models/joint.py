...
class Joint(_PrevJoint):
    def __init__(
        ...
        super().__init__(name=name, type=type, parent=parent, child=child, pose=pose, thread_pitch=thread_pitch,
                         axis=axis, axis2=axis2, physics=physics)
    ...
    def from_sdf(cls, el: ET.Element) -> "Joint":
        ...
        return cls(name=_base.name, type=_base.type, sensor=_sensor, parent=_base.parent, child=_base.child,
                   pose=_base.pose, gearbox_ratio=_gearbox_ratio, gearbox_reference_body=_gearbox_reference_body,
