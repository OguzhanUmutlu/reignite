...
class Physics(_PrevPhysics):
    ...
    def from_sdf(cls, el: ET.Element) -> "Physics":
        ...
        return cls(type=_type, max_step_size=_max_step_size, real_time_factor=_real_time_factor, real_time_update_rate=_real_time_update_rate, max_contacts=_max_contacts, gravity=_gravity, simbody=_simbody, bullet=_bullet, ode=_base.ode)
...
class World(_PrevWorld):
    ...
    def from_sdf(cls, el: ET.Element) -> "World":
        ...
        _spherical_coordinates = SphericalCoordinates.from_sdf(_c_spherical_coordinates) if _c_spherical_coordinates is not None else None
        ...
        return cls(name=_base.name, gui=_base.gui, physics=_base.physics, scene=_base.scene, light=_base.light, model=_base.model, actor=_base.actor, plugin=_base.plugin, joint=_base.joint, road=_base.road, spherical_coordinates=_spherical_coordinates, state=_base.state, audio=_audio, include=_include)
