...
class Inertial(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        if _c_pose is not None:
            ...
            _pose = None
        return cls(sdf_version=version, frame=_frame, inertia=_inertia, mass=_mass, pose=_pose)
...
class Mesh(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        if _c_uri is not None:
            ...
            _uri = None
        return cls(sdf_version=version, filename=_filename, scale=_scale, submesh=_submesh, uri=_uri)
...
class Geometry(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        if _c_sphere is not None:
            ...
            _sphere = None
        return cls(sdf_version=version, box=_box, capsule=_capsule, cylinder=_cylinder, ellipsoid=_ellipsoid, empty=_empty, heightmap=_heightmap, image=_image, mesh=_mesh, plane=_plane, polyline=_polyline, sphere=_sphere)
...
class Collision(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        if _c_surface is not None:
            ...
            _surface = None
        return cls(sdf_version=version, frame=_frame, geometry=_geometry, laser_retro=_laser_retro, max_contacts=_max_contacts, name=_name, pose=_pose, surface=_surface)
...
class Visual(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        if _visibility_flags is not None and cmp_version(version, "1.7") < 0:
            return SDFError(f"'visibility_flags' is not supported in SDF version {version} (added in 1.7)")
        return cls(sdf_version=version, cast_shadows=_cast_shadows, frame=_frame, geometry=_geometry, laser_retro=_laser_retro, material=_material, meta=_meta, name=_name, plugin=_plugin, pose=_pose, transparency=_transparency, visibility_flags=_visibility_flags)
...
class Clip(BaseModel):
    def __init__(self, sdf_version: str, far: "Far" = None, near: "Near" = None):
        self.__version__ = sdf_version
        self.far = far
        self.near = near
...
class DepthCamera(BaseModel):
    def __init__(self, sdf_version: str, clip: "DepthCameraClip" = None, output: "Output" = None):
        self.__version__ = sdf_version
        self.clip = clip
        self.output = output
...
class Sensor(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        if _c_visualize is not None:
            ...
            _visualize = None
        return cls(sdf_version=version, air_pressure=_air_pressure, altimeter=_altimeter, always_on=_always_on, camera=_camera, contact=_contact, enable_metrics=_enable_metrics, force_torque=_force_torque, frame=_frame, gps=_gps, imu=_imu, lidar=_lidar, logical_camera=_logical_camera, magnetometer=_magnetometer, name=_name, navsat=_navsat, plugin=_plugin, pose=_pose, ray=_ray, rfid=_rfid, rfidtag=_rfidtag, sonar=_sonar, topic=_topic, transceiver=_transceiver, type=_type, update_rate=_update_rate, visualize=_visualize)
...
class Light(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        if _visualize is not None and cmp_version(version, "1.8") < 0:
            return SDFError(f"'visualize' is not supported in SDF version {version} (added in 1.8)")
        return cls(sdf_version=version, attenuation=_attenuation, cast_shadows=_cast_shadows, diffuse=_diffuse, direction=_direction, frame=_frame, intensity=_intensity, light_on=_light_on, name=_name, pose=_pose, specular=_specular, spot=_spot, type=_type, visualize=_visualize)
...
class Link(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        for c in el.findall("visual"):
            ...
            _visual.append(_res)
        return cls(sdf_version=version, audio_sink=_audio_sink, audio_source=_audio_source, battery=_battery, collision=_collision, enable_wind=_enable_wind, frame=_frame, gravity=_gravity, inertial=_inertial, kinematic=_kinematic, light=_light, must_be_base_link=_must_be_base_link, name=_name, particle_emitter=_particle_emitter, pose=_pose, projector=_projector, self_collide=_self_collide, sensor=_sensor, velocity_decay=_velocity_decay, visual=_visual)
...
class Axis(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        if _c_xyz is not None:
            ...
            _xyz = None
        return cls(sdf_version=version, dynamics=_dynamics, initial_position=_initial_position, limit=_limit, use_parent_model_frame=_use_parent_model_frame, xyz=_xyz)
...
class Joint(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        if isinstance(_type, SDFError):
            return _type.extend("@type")
        return cls(sdf_version=version, axis=_axis, axis2=_axis2, child=_child, frame=_frame, gearbox_ratio=_gearbox_ratio, gearbox_reference_body=_gearbox_reference_body, name=_name, parent=_parent, physics=_physics, pose=_pose, sensor=_sensor, thread_pitch=_thread_pitch, type=_type)
