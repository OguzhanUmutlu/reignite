...
class Visual(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        if _visibility_flags is not None and cmp_version(version, "1.7") < 0:
            return SDFError(f"'visibility_flags' is not supported in SDF version {version} (added in 1.7)")
        return cls(sdf_version=version, name=_name, geometry=_geometry, cast_shadows=_cast_shadows, laser_retro=_laser_retro, transparency=_transparency, pose=_pose, material=_material, plugin=_plugin, meta=_meta, frame=_frame, visibility_flags=_visibility_flags)
...
class Sensor(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        return cls(sdf_version=version, name=_name, type=_type, plugin=_plugin, camera=_camera, ray=_ray, contact=_contact, rfidtag=_rfidtag, rfid=_rfid, always_on=_always_on, update_rate=_update_rate, visualize=_visualize, pose=_pose, topic=_topic, imu=_imu, sonar=_sonar, force_torque=_force_torque, transceiver=_transceiver, gps=_gps, magnetometer=_magnetometer, frame=_frame, logical_camera=_logical_camera, altimeter=_altimeter, air_pressure=_air_pressure, lidar=_lidar, enable_metrics=_enable_metrics, navsat=_navsat)
...
class Light(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        if _visualize is not None and cmp_version(version, "1.8") < 0:
            return SDFError(f"'visualize' is not supported in SDF version {version} (added in 1.8)")
        return cls(sdf_version=version, name=_name, type=_type, frame=_frame, pose=_pose, cast_shadows=_cast_shadows, diffuse=_diffuse, specular=_specular, attenuation=_attenuation, direction=_direction, spot=_spot, light_on=_light_on, intensity=_intensity, visualize=_visualize)
...
class Link(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        return cls(sdf_version=version, name=_name, inertial=_inertial, collision=_collision, visual=_visual, sensor=_sensor, projector=_projector, gravity=_gravity, self_collide=_self_collide, kinematic=_kinematic, pose=_pose, velocity_decay=_velocity_decay, audio_source=_audio_source, audio_sink=_audio_sink, must_be_base_link=_must_be_base_link, battery=_battery, frame=_frame, enable_wind=_enable_wind, light=_light, particle_emitter=_particle_emitter)
...
class JointSensor(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        if _navsat is not None and cmp_version(version, "1.7") < 0:
            return SDFError(f"'navsat' is not supported in SDF version {version} (added in 1.7)")
        return cls(sdf_version=version, name=_name, type=_type, plugin=_plugin, camera=_camera, contact=_contact, force_torque=_force_torque, gps=_gps, imu=_imu, ray=_ray, rfidtag=_rfidtag, rfid=_rfid, sonar=_sonar, transceiver=_transceiver, always_on=_always_on, update_rate=_update_rate, visualize=_visualize, pose=_pose, topic=_topic, magnetometer=_magnetometer, frame=_frame, logical_camera=_logical_camera, altimeter=_altimeter, air_pressure=_air_pressure, lidar=_lidar, enable_metrics=_enable_metrics, navsat=_navsat)
...
class Joint(BaseModel):
    ...
    def _from_sdf(cls, el: ET.Element, version: str):
        ...
        if _frame and cmp_version(version, "1.5") < 0:
            return SDFError(f"'frame' is not supported in SDF version {version} (added in 1.5)")
        return cls(sdf_version=version, name=_name, type=_type, parent=_parent, child=_child, pose=_pose, thread_pitch=_thread_pitch, axis=_axis, axis2=_axis2, physics=_physics, sensor=_sensor, gearbox_reference_body=_gearbox_reference_body, gearbox_ratio=_gearbox_ratio, frame=_frame)
