...
class Geometry(Model):
    ...
    def from_sdf(cls, el: ET.Element, version: str) -> "Geometry":
        ...
        return cls(sdf_version=version, box=_box, sphere=_sphere, cylinder=_cylinder, mesh=_mesh, plane=_plane, image=_image, heightmap=_heightmap, empty=_empty, polyline=_polyline, capsule=_capsule, ellipsoid=_ellipsoid)
...
class Ode(Model):
    ...
    def from_sdf(cls, el: ET.Element, version: str) -> "Ode":
        _c_mu = el.find("mu")
        _mu = Mu.from_sdf(_c_mu, version) if _c_mu is not None else None
        _c_mu2 = el.find("mu2")
        _mu2 = Mu2.from_sdf(_c_mu2, version) if _c_mu2 is not None else None
        _c_fdir1 = el.find("fdir1")
        _fdir1 = Fdir1.from_sdf(_c_fdir1, version) if _c_fdir1 is not None else None
        _c_slip1 = el.find("slip1")
        _slip1 = Slip1.from_sdf(_c_slip1, version) if _c_slip1 is not None else None
        _c_slip2 = el.find("slip2")
        _slip2 = Slip2.from_sdf(_c_slip2, version) if _c_slip2 is not None else None
...
class Sensor(Model):
    ...
    def from_sdf(cls, el: ET.Element, version: str) -> "Sensor":
        ...
        if _enable_metrics is not None and cmp_version(version, "1.7") < 0:
            raise ValueError(f"'enable_metrics' is not supported in SDF version {version} (added in 1.7)")
        return cls(sdf_version=version, name=_name, type=_type, plugin=_plugin, camera=_camera, ray=_ray, contact=_contact, rfidtag=_rfidtag, rfid=_rfid, always_on=_always_on, update_rate=_update_rate, visualize=_visualize, pose=_pose, topic=_topic, imu=_imu, force_torque=_force_torque, gps=_gps, sonar=_sonar, transceiver=_transceiver, frame=_frame, altimeter=_altimeter, logical_camera=_logical_camera, magnetometer=_magnetometer, air_pressure=_air_pressure, lidar=_lidar, navsat=_navsat, enable_metrics=_enable_metrics)
...
class Joint(Model):
    ...
    def from_sdf(cls, el: ET.Element, version: str) -> "Joint":
        ...
        if _frame and cmp_version(version, "1.5") < 0:
            raise ValueError(f"'frame' is not supported in SDF version {version} (added in 1.5)")
        return cls(sdf_version=version, name=_name, type=_type, parent=_parent, child=_child, pose=_pose, thread_pitch=_thread_pitch, axis=_axis, axis2=_axis2, physics=_physics, sensor=_sensor, gearbox_ratio=_gearbox_ratio, gearbox_reference_body=_gearbox_reference_body, frame=_frame)
...
class Robot(Model):
    ...
    @classmethod
    def from_sdf(cls, el: ET.Element, version: str) -> "Robot":
        _name = el.get("name", "__default__")
