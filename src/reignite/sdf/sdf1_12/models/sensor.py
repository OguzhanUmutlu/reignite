...
from ...sdf1_11.models.sensor import Sensor as _PrevSensor
...
class Sensor(_PrevSensor):
    def __init__(
        ...
        super().__init__(name=name, type=type, pose=pose, plugin=plugin, air_pressure=air_pressure, air_speed=air_speed,
                         altimeter=altimeter, camera=camera, contact=contact, force_torque=force_torque, gps=gps,
                         imu=imu, lidar=lidar, logical_camera=logical_camera, magnetometer=magnetometer, navsat=navsat,
                         ray=ray, rfidtag=rfidtag, rfid=rfid, sonar=sonar, transceiver=transceiver, always_on=always_on,
                         update_rate=update_rate, visualize=visualize, topic=topic, enable_metrics=enable_metrics)
        self.frame_id = frame_id

    def to_sdf(self) -> ET.Element:
        el = super().to_sdf()
        if self.frame_id is not None:
            el.append(self.frame_id.to_sdf())
        return el
    ...
    def from_sdf(cls, el: ET.Element) -> "Sensor":
        ...
        _frame_id = FrameId.from_sdf(_c_frame_id) if _c_frame_id is not None else None
        return cls(name=_base.name, type=_base.type, pose=_base.pose, plugin=_base.plugin,
                   air_pressure=_base.air_pressure, air_speed=_base.air_speed, altimeter=_base.altimeter,
                   camera=_base.camera, contact=_base.contact, force_torque=_base.force_torque, gps=_base.gps,
                   imu=_base.imu, lidar=_base.lidar, logical_camera=_base.logical_camera,
                   magnetometer=_base.magnetometer, navsat=_base.navsat, ray=_base.ray, rfidtag=_base.rfidtag,
                   rfid=_base.rfid, sonar=_base.sonar, transceiver=_base.transceiver, always_on=_base.always_on,
                   update_rate=_base.update_rate, visualize=_base.visualize, topic=_base.topic,
                   enable_metrics=_base.enable_metrics, frame_id=_frame_id)
