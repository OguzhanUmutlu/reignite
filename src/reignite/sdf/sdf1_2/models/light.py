...
class Light(_PrevLight):
    def __init__(
        ...
        super().__init__(name=name, type=type, cast_shadows=cast_shadows, diffuse=diffuse, specular=specular, attenuation=attenuation, direction=direction, spot=spot)
        self.pose = pose
    ...
    def from_sdf(cls, el: ET.Element) -> "Light":
        ...
        return cls(name=_base.name, type=_base.type, cast_shadows=_base.cast_shadows, pose=_pose, diffuse=_base.diffuse, specular=_base.specular, attenuation=_base.attenuation, direction=_base.direction, spot=_base.spot)
