...
from ...sdf1_10.models.link import Link as _PrevLink
...
class Link(_PrevLink):
    def __init__(
        ...
        super().__init__(name=name, pose=pose, inertial=inertial, collision=collision, visual=visual, sensor=sensor, projector=projector, audio_sink=audio_sink, audio_source=audio_source, battery=battery, light=light, particle_emitter=particle_emitter, gravity=gravity, enable_wind=enable_wind, self_collide=self_collide, kinematic=kinematic, must_be_base_link=must_be_base_link, velocity_decay=velocity_decay)
    ...
    def from_sdf(cls, el: ET.Element) -> "Link":
        ...
        return cls(name=_base.name, pose=_base.pose, inertial=_base.inertial, collision=_base.collision, visual=_base.visual, sensor=_base.sensor, projector=_base.projector, audio_sink=_base.audio_sink, audio_source=_base.audio_source, battery=_base.battery, light=_base.light, particle_emitter=_base.particle_emitter, gravity=_base.gravity, enable_wind=_base.enable_wind, self_collide=_base.self_collide, kinematic=_base.kinematic, must_be_base_link=_base.must_be_base_link, velocity_decay=_base.velocity_decay)
