...
from ...sdf1_10.models.audio_source import AudioSource as _PrevAudioSource
...
class AudioSource(_PrevAudioSource):
    def __init__(
        self,
        pose: "Pose" = None,
        uri: "Uri" = None,
        ...
        super().__init__(pose=pose, uri=uri, pitch=pitch, gain=gain, contact=contact, loop=loop)
    ...
    def from_sdf(cls, el: ET.Element) -> "AudioSource":
        _base = _PrevAudioSource.from_sdf(el)
        return cls(pose=_base.pose, uri=_base.uri, pitch=_base.pitch, gain=_base.gain, contact=_base.contact, loop=_base.loop)
